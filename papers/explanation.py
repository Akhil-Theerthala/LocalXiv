...
def validate_scene(scene, evidence=None):
    """Validate one Overview scene tree. Returns a deep copy or raises ``PanelPlanError``."""
    errors = []
    if not isinstance(scene, dict):
        raise PanelPlanError([{'code': 'scene_validation', 'path': 'scene', 'message': 'scene must be an object.'}])
    ...
    for name in ('title', 'subtitle', 'footer'):
        _text(scene, name, 'scene', errors, maximum=SCENE_LIMITS[name])
    ...
    if not isinstance(panels, list) or not 1 <= len(panels) <= SCENE_MAX_PANELS:
        _panel_error(errors, 'scene.panels', f'needs 1 through {SCENE_MAX_PANELS} panels',
                     constraint='maximum_panels', actual=len(panels) if isinstance(panels, list) else None,
                     limit=SCENE_MAX_PANELS)
    ...
    for index, panel in enumerate(panels):
        ...
        if not isinstance(notes, list) or len(notes) > 2 or any(
                not isinstance(line, str) or not line.strip() or len(line) > SCENE_LIMITS['panel_note'] for line in notes):
            _panel_error(errors, path + '.notes', f'needs at most 2 lines of 1-{SCENE_LIMITS["panel_note"]} characters')
        edges = panel.get('edges', [])
        if not isinstance(edges, list) or len(edges) > SCENE_MAX_EDGES:
            _panel_error(errors, path + '.edges', f'needs at most {SCENE_MAX_EDGES} arrows')
        ...
        for position, edge in enumerate(edges):
            ...
            if 'label' in edge:
                _text(edge, 'label', edge_path, errors, maximum=SCENE_LIMITS['edge_label'])
    if errors:
        raise PanelPlanError(errors[:20])
...
def _scene_node(node, path, depth, ids, count, errors):
    ...
    if kind not in SCENE_KINDS:
        _panel_error(errors, path + '.kind', 'must be one of ' + ', '.join(SCENE_KINDS))
        return
    for name in sorted(set(node) - SCENE_NODE_FIELDS[kind]):
        _panel_error(errors, path + '.' + name, 'is unsupported')
    if kind != 'group':
        count[0] += 1
    ...
    if kind == 'card':
        ...
        if 'detail' in node:
            _text(node, 'detail', path, errors, maximum=SCENE_LIMITS['detail'])
        _scene_id(node, path, ids, errors)
    elif kind == 'group':
        if depth > SCENE_MAX_DEPTH:
            _panel_error(errors, path, f'nests deeper than {SCENE_MAX_DEPTH} groups')
            return
        if 'heading' in node:
            _text(node, 'heading', path, errors, maximum=SCENE_LIMITS['group_heading'])
        if 'repeat' in node:
            _text(node, 'repeat', path, errors, maximum=SCENE_LIMITS['repeat'])
        ...
        _scene_lines(node.get('lines'), path + '.lines', errors, maximum=4, length=SCENE_LIMITS['note_line'])
    elif kind == 'sequence':
        ...
        for index, item in enumerate(items):
            ...
            _text(item, 'text', item_path, errors, maximum=SCENE_LIMITS['item'])
            if 'sub' in item:
                _text(item, 'sub', item_path, errors, maximum=SCENE_LIMITS['sub'])
            if 'tone' in item and item['tone'] not in SCENE_TONES:
                _panel_error(errors, item_path + '.tone', 'must be one of ' + ', '.join(SCENE_TONES))
            ...
            for r, row in enumerate(rows):
                for c, cell in enumerate(row):
                    if cell is not None and not isinstance(cell, (int, float)) and (
                            not isinstance(cell, str) or len(cell.lstrip('*')) > SCENE_LIMITS['cell']):
        ...
        _scene_lines(node.get('lines'), path + '.lines', errors, maximum=6, length=SCENE_LIMITS['step'])
    elif kind == 'bars':
        items = node.get('items')
        if not isinstance(items, list) or not 2 <= len(items) <= 8 or any(
                ...
                or not item[0].strip() or len(item[0]) > SCENE_LIMITS['bar_label']
        ...
        if 'label' in node:
            _text(node, 'label', path, errors, maximum=SCENE_LIMITS['divider'])
