...
def build_orientation(document):
    ...
    for index,passage in enumerate(passages,1):
        ...
        for element in _ancestors(target,parents) if target is not None else ():
            if element.tag.split('}')[-1] in ('section','article'):
        ...
        if is_pdf:
            ...
            current['passages'].append(passage['id'])
            current['chars']+=len(passage.get('text',''))
            continue
        ...
        if structure is not None:
            mapped_structures[structure]=section['id']
    ...
    warnings=[]
    if ambiguous:
        warnings.append('Abstract structure is ambiguous at passages: '+', '.join(ambiguous))
    if not abstract and not ambiguous:
        warnings.append('No explicit abstract was identified.')
    if is_pdf and not figures:
        warnings.append('Original figure images are unavailable in the retained PDF text index.')
...
def _selected_image_bytes(document,passage):
    trees={}
    source,target,parents=_xhtml_target(document,passage,trees)
    if target is None:
        return [],'missing retained figure location'
    figure=next((item for item in _ancestors(target,parents)
                 if item.tag.split('}')[-1]=='figure'),None)
    if figure is None:
        return [],'selected source is not a raster figure'
    root=Path(document['directory']).resolve()
    images=[]
    problem='missing image asset'
    for element in figure.iter():
        ...
        if src.scheme or src.netloc or not path.is_relative_to(root/'reader'):
            problem='external or unsafe image path'
            continue
        try:
...
def retrieve_evidence(document,orientation,selection,*,vision):
    ...
    def selected_section(section):
        ...
        while current is not None:
            if current['id'] in chosen_sections:
                return True
    ...
    for section in orientation['sections']:
        if selected_section(section):
            passage_ids.update(section['passages'])
    ...
    if vision:
        ...
        for figure_id in selection['figure_ids']:
            ...
            if reason:
                unavailable.append({'figure_id':figure_id,'reason':reason})
            for image in found:
                if len(images)>=6:
                    unavailable.append({'figure_id':figure_id,'reason':'six-image attachment limit'})
                    break
                images.append(image)
                if figure_id not in attached_ids:
                    attached_ids.append(figure_id)
    else:
