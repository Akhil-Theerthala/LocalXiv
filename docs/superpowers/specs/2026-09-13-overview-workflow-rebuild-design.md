...
This design replaces the Overview requirements in ADRs 0001–0003 and the amended `2026-09-13-model-authored-svg-overviews.md` plan where they conflict. The earlier `docs/workflow_plan.md`, `docs/workflow_plan_r2.md`, and `progress_summary_1309.md` are in git history.
...
- Panel authors receive relevant SVG examples, concise construction references, and bounded shared story orientation from the accepted narrative, without the paper, planning history, or other panel briefs.
- The first panel-plan refinement clarifies. The last allocated refinement corrects remaining structural defects while preserving valid content and handoffs, or simplifies unresolved semantic dependencies when the plan is structurally valid.
...
2. Narrative planning identifies the central contribution, mechanism, finding, and qualification for the target reader. A second call may correct the narrative, carrying the rejected answer back as an assistant message and preserving valid claims, citations, and relationships. The Overview narrative has its own validator: nonempty text without Blog's per-field length cap, a bounded 256 KiB candidate reported as a resource limit, relationships optional but every supplied one validated. If both answers are rejected, one candidate whose four claims are valid and source-linked may be recovered by preserving its valid focus (including an ordered string list normalized to text), deriving focus from the contribution only when needed, and dropping only invalid optional relationships. Malformed relationship containers cannot defeat this recovery; valid source-linked relationships survive. Recovery provenance distinguishes preserved from derived focus, and no field is ever borrowed from another candidate. Blog keeps its existing `validate_plan` contract.
3. Panel planning assigns that narrative to 1–7 detailed briefs. Its first refinement checks and clarifies the initial briefs. The final allocated pass receives the latest rejected candidate and structural diagnostics when correction is needed; it must preserve valid panels, content, shared facts, and handoffs rather than request dependency removal. For a structurally valid plan with unresolved semantic issues, that pass instead simplifies affected connections into self-contained panels. An issue-free valid draft survives a malformed clarification. Review and revision stay within these planner passes; there is no added request or repeating draft-review agent.
...
| `construction` | Reference family: `flow`, `mapping`, `comparison`, `calculation`, or `chart`; not a mandatory template |
| `layout_intent` (optional) | The visual inference to teach, such as the same object before/after or contrasting paths; not rigid coordinates |
...
Overview assignments also project optional `layout_intent` and, when the accepted narrative is supplied, identical bounded `story_context` for every author and local repair. Story context is teaching orientation, not new drawing evidence or exact display text. Whole normal-size focus/contribution fields are preferred; oversized fields select complete fitting sentences, never a partial scientific expression. If neither primary field supplies bounded orientation, a complete bounded question/finding/limitation may be used; otherwise an explicit omission notice is sent. Inline evidence handles are stripped only from this projection, not saved claims or exact panel content. Existing plans and callers without these optional inputs remain valid; Blog's request structure is unchanged.
...
Semantic recovery is clarify, then simplify; structural recovery instead corrects only reported contract defects within that same allocation. Semantic simplification removes unresolved dependencies and gives each affected panel sufficient source-supported content to stand alone. It is not another polish loop. A plan is accepted only when structural validation and its reported issue list are both clear; repeated identical candidates cannot clear unresolved semantic reports merely by returning an empty issue list. If correction drops valid panels or handoffs, the loss is disclosed as reduced planning rather than ordinary success. Mismatched exact text, unknown source references, and conflicting science must not be silently deleted to pass validation.
