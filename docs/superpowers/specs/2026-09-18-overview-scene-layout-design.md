...
- 2026-09-24: in six deepseek-flash Overview runs, arrows shared lines with arrows between other cards (an arrow entering a card where another left it), passed through groups that held neither end, and once found no clear candidate, so the whole Overview failed. `route.search` finds a path along the lanes in those cases (`tests/test_arrow_routes.py`); an arrow with a clear candidate that has none of these defects keeps it. The reference Scene's Level 1 now lays out as a 640-unit Blog panel, where it raised before. Open: arrows that enter one card can still cross each other, and a last run in a 14-unit gap is still too short for the arrowhead.
