...
- In `papers/explanation.py`: `validate_scene`, `scene_coverage_issues`, the `SCENE_*` constants, `PANEL_CONSTRUCTION_FAMILIES`, `blog_figure_assignment`, and the `construction` and `layout_intent` fields of `BLOG_BRIEF_SCHEMA`. Check: `grep -n "SCENE_\|construction\|layout_intent" papers/explanation.py` prints nothing.
- In `papers/overview_workflow.py`: `SCENE_INSTRUCTION`, both example constants, and the coordinator classes after they move. Check: `grep -n "SCENE_INSTRUCTION\|class Coordinator\|class RunStore" papers/overview_workflow.py` prints nothing.
