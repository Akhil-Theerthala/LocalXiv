...
def main():
    ...
    if not manifest.get('frozen_at'):
        parser.error('Sample is not frozen')
    ...
    if args.ids and set(args.ids)-{p['id'] for p in papers}:
        parser.error('Unknown ID or wrong split')
    for paper in papers:
        identifier=paper['id']
        ...
        if revision(code)!=fingerprint:
            raise ValueError('Parser changed during evaluation; stop and start a new stage')
        ...
        if revision(code)!=fingerprint:
            record['status']='invalidated_code_changed'
        results[identifier]=record
        save(result_path,results)
