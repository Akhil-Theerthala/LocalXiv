...
def main():
    ...
    if manifest.get('frozen_at'):
        print('ALREADY FROZEN',len(manifest['papers']))
        return
    ...
    for name,cat in fields:
        ...
        year=rng.choice(list(range(2020,2026)))
        ...
        if not count:
            raise ValueError('No monthly population count: '+base)
        total=int(count[1].replace(',',''))
        pool=parse(page,provenance)
        for start in range(2000,total,2000):
            page,provenance=fetch(base+f'?skip={start}&show=2000')
            pool.extend(parse(page,provenance))
        pool=sorted({p['id']:p for p in pool}.values(),key=lambda p:p['id'])
        if len(pool)!=total:
            raise ValueError(f'Listing incomplete: {base}: {len(pool)} of {total}')
        save(CACHE/'pools'/f'{name}-listing.json',pool)
        for p in pool:
            if re.search(r'\bnotes\b|\blectures?\b|\btutorial\b',p['title']+' '+p['comment'],re.I):
                note_pool[p['id']]=p
        if name in done:
            continue
        selected=[]
    ...
    if 'notes' not in done:
        ...
        rng=random.Random(f'{SEED}:listing:notes')
        selected=[]
        for rank in rng.sample(range(len(pool)),len(pool)):
            ...
            if len(selected)==12:
                break
        if len(selected)!=12:
            raise ValueError('Insufficient distinct notes')
        for p in selected[-3:]:
            p['split']='holdout'
